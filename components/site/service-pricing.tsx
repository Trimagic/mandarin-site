...
function PricingAction({ href, className, children, "aria-label": ariaLabel }: { href: string; className?: string; children: ReactNode; "aria-label"?: string }) {
  if (isEnquiryHref(href)) return <RequestTrigger fallbackHref={href} aria-label={ariaLabel} className={className}>{children}</RequestTrigger>;
  return <a href={href} aria-label={ariaLabel} className={className}>{children}</a>;
}
    ...
    <section aria-labelledby={headingId} className="mx-auto w-full max-w-[1440px] px-5 pb-10 md:px-6 xl:px-12">
      <div className={cn("grid gap-5 lg:gap-8", data.callout && "lg:grid-cols-[1.05fr_1fr]")}>
          ...
          <ul className="grid grid-cols-[minmax(0,1fr)_auto] divide-y divide-[#e6e2de] overflow-hidden rounded-[6px] border border-[#e6e2de] bg-[#fffefd] sm:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)_auto] dark:divide-[#46301f] dark:border-[#46301f] dark:bg-[#15110e]">
            {data.items.map((item) => (
              <li key={item.id} className="col-span-full grid grid-cols-subgrid items-center gap-x-4 gap-y-1 px-4 py-3 sm:px-5">
                ...
                {item.action && (
                  <PricingAction href={item.action.href} aria-label={`${requestLabel(item.action.label)}: ${item.title}`} className="col-start-2 row-start-1 row-end-3 inline-flex min-h-11 items-center gap-1 rounded-sm text-xs font-medium text-[#ff5000] hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#ff5000] sm:col-start-auto sm:row-auto">
                    {isEnquiryHref(item.action.href) ? requestLabel(item.action.label) : item.action.label}<IconChevronRight aria-hidden="true" className="size-4 shrink-0" />
                  </PricingAction>
                )}
              </li>
            ))}
        ...
        {data.callout && <div className="relative isolate flex flex-col justify-center overflow-hidden rounded-[8px] bg-[#ff5000] p-6 text-white sm:p-8">
          ...
          <PricingAction href={data.callout.action.href} className={cn(buttonVariants({ variant: "secondary", size: "xl" }), "mt-5 h-11 w-fit max-w-full whitespace-normal rounded-md bg-white px-5 text-center text-xs text-[#e74700] hover:bg-[#fff3eb] focus-visible:ring-white/70")}>{isEnquiryHref(data.callout.action.href) ? requestLabel(data.callout.action.label) : data.callout.action.label}</PricingAction>
        </div>}
      </div>
    </section>
  );
